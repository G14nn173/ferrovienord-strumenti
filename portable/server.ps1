...
try {
  ...
  Write-Host ""
  Write-Host "FERROVIENORD - Mappa interattiva V1" -ForegroundColor Cyan
  Write-Host "Aperta nel browser: $prefix"
  Write-Host "Lascia aperta questa finestra. Premi Ctrl+C per chiudere la mappa."
  Write-Host ""
  ...
  while ($true) {
    ...
    try {
      ...
      # arrestare il server della mappa.
    } finally {
      $client.Close()
    }
  }
